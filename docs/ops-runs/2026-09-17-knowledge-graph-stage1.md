...
功能目标提交：`5495d9f`（`feat: deliver knowledge graph stage one`）；候选最终 HEAD：`821992a`（报告校准提交）。当前分支相对 `origin/main=52acab2` 多 5 个候选提交，工作树干净。回滚只需移除本阶段的服务、路由、共享模板、样式、测试和报告提交，不需要生产 schema 回滚；主工作区原有未提交改动不在候选回滚范围内。
