...
- 本次审查指定定向命令：`python -m pytest tests/test_knowledge_eval.py tests/test_knowledge_vector_store.py tests/test_knowledge_rag.py -q --disable-warnings` 为 `19 passed`，退出码 0，37.93s。
...
- 固定评估的普通问题耗时：切分/索引构建 mean 0.341ms、P95 1.477ms；查询 mean 0.045ms、P95 0.094ms；合计 mean 0.387ms、P95 1.532ms。
- 固定 64 文档性能样本、64 个切片、100 次查询、top-k=8：构建 0.946ms；查询 mean 0.392ms、P95 0.660ms；总耗时 40.161ms，按运行摊销 0.402ms/次。
